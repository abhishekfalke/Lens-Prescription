...
public class InsightsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Wire bottom nav — Insights is active
        BottomNavHelper.setup(this, BottomNavHelper.NAV_INSIGHTS);
    }
    ...
    private void computeAndDisplay(List<Prescription> list) {
        ...
        float sumR = 0, sumL = 0;
        for (Prescription p : list) { sumR += p.getRightSphere(); sumL += p.getLeftSphere(); }
        ...
        for (Prescription p : list) {
            if (p.getRightAxis() >= 0 && p.getRightAxis() <= 180) axisCount[p.getRightAxis()]++;
            if (p.getLeftAxis()  >= 0 && p.getLeftAxis()  <= 180) axisCount[p.getLeftAxis()]++;
        }
    ...
    private String buildTrendText(float rightChange, float leftChange, int n) {
        ...
        sb.append(String.format(Locale.getDefault(),
                ...
                n, Prescription.formatDiopter(rightChange), Prescription.formatDiopter(leftChange)));
